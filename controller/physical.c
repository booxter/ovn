...
static void
handle_oversized_ip_packets_direction(struct ovn_desired_flow_table *flow_table,
                                      const struct sbrec_port_binding *binding,
                                      uint32_t port_key, bool is_ipv6, bool ingress)
{
    uint32_t dp_key = binding->datapath->tunnel_key;
    ...
    match_init_catchall(&match);
    /* Use REGBIT_SIZE_ASSESSED as a singleton tag to visit once. */
    match_set_reg_masked(&match, MFF_REG9 - MFF_REG0, 0, 1 << 6);
    match_set_dl_type(&match, htons(is_ipv6 ? ETH_TYPE_IPV6 : ETH_TYPE_IP));
    match_set_metadata(&match, htonll(dp_key));
    match_set_reg(&match, (ingress ? MFF_LOG_INPORT : MFF_LOG_OUTPORT) - MFF_REG0, port_key);
    ...
    ovs_be32 value = htonl(1 << 6);
    ovs_be32 mask = htonl(1 << 6);
    ofpact_put_set_field(
        &ofpacts, mf_from_id(MFF_REG9), &value, &mask);
    ...
    /* TODO: why ipv6 addresses still zeroed? */
...
handle_oversized_ip_packets(struct ovn_desired_flow_table *flow_table,
                            const struct sbrec_port_binding *binding,
                            uint32_t port_key, bool is_ipv6)
{
    handle_oversized_ip_packets_direction(flow_table, binding, port_key, is_ipv6, true);
    handle_oversized_ip_packets_direction(flow_table, binding, port_key, is_ipv6, false);
}
    ...
    struct ovn_desired_flow_table *flow_table)
{
    ...
    SHASH_FOR_EACH (node, &ld->multichassis_ports) {
        ...
        handle_oversized_ip_packets(flow_table, binding, mcp->tunnel_key, false);
        handle_oversized_ip_packets(flow_table, binding, mcp->tunnel_key, true);
    }
