    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    ports_to_activate_in_engine = xmalloc(sizeof *ports_to_activate_in_engine);
...
void
tag_ports_as_activated_in_engine(struct ovs_list *activated_ports) {
    if (!activated_ports) {
        return;
    }
    ovs_mutex_lock(&pinctrl_mutex);
    struct activated_port *pp, *ap;
    /* TODO: convert to set */
    LIST_FOR_EACH_POP(ap, list, activated_ports) {
