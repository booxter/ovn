...
static void
update_tracked_lports_for_activated_ports(struct engine_node *node,
                                          struct ovs_list *activated_ports,
                                          struct ed_type_runtime_data *rt_data)
{
    if (!activated_ports) {
...
en_runtime_data_run(struct engine_node *node, void *data)
{
    ...
    struct ed_type_activated_ports *ap_data =
        engine_get_input_data("activated_ports", node);
    update_tracked_lports_for_activated_ports(node, ap_data->activated_ports, rt_data);
...
runtime_data_activated_ports_handler(struct engine_node *node, void *data)
{
    ...
    struct ed_type_activated_ports *ap =
        engine_get_input_data("activated_ports", node);
    update_tracked_lports_for_activated_ports(node, ap->activated_ports, rt_data);
    return true;
}
            ...
            if (br_int) {
                ...
                if (chassis) {
                    ...
                    if (lflow_output_data && pflow_output_data &&
                        ...
                        struct ed_type_activated_ports *ap_data = engine_get_data(
                            &en_activated_ports);
                        tag_ports_as_activated_in_engine(ap_data->activated_ports);
                        free(ap_data->activated_ports);
                        ap_data->activated_ports = NULL;
                    }
